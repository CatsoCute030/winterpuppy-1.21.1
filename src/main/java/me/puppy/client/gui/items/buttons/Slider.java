...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderWorldUtil.rect(context.getMatrices(), this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515);
        RenderWorldUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Winterpuppy.colorManager.getColorWithAlpha(Winterpuppy.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Winterpuppy.colorManager.getColorWithAlpha(Winterpuppy.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        drawString(this.getName() + " " + Formatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf((this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) WinterGui.getClickGui().getTextOffset(), -1);
    }
