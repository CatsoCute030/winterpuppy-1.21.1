...
import me.puppy.system.utils.Timer;
...
public class StringButton
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            drawString(this.currentString.string() + "_", this.x + 2.3f, this.y - 1.7f - (float) WinterGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            drawString((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + Formatting.GRAY : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) WinterGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
    ...
    public void onKeyTyped(char typedChar, int keyCode) {
        if (this.isListening) {
            char[] chars = new char[]{'/', '\n', '\r', '\t', '\u0000', '\f', '`', '?', '*', '\\', '<', '>', '|', '"', ':'};
            ArrayList<Character> characters = (ArrayList<Character>) new String(chars)
                    .chars()  // 將 char[] 轉為 IntStream
                    .mapToObj(ca -> (char) ca)  // 將 int 轉為 char
                    .toList();
            if (!characters.contains(typedChar)) {
